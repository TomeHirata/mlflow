...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("https://ipv6.internal/hook", "::1"),
        ("https://ipv6.internal/hook", "fc00::1"),
    ],
)
def test_validate_webhook_url_rejects_non_public_ips(url, resolved_ip):
    with patch(
        "mlflow.utils.validation.socket.getaddrinfo",
        side_effect=_mock_getaddrinfo(resolved_ip),
    ):
        with pytest.raises(MlflowException, match="non-public IP address"):
            _validate_webhook_url(url)
...
def test_validate_webhook_url_rejects_if_any_resolved_address_is_non_public():
