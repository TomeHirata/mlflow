...
def test_test_webhook_rejects_non_public_ip():
    def mock_addr(host, port, *a, **kw):
    ...
    with patch("mlflow.utils.validation.socket.getaddrinfo", side_effect=mock_addr):
        result = send_test_webhook(webhook)

    assert result.success is False
    assert "non-public IP address" in result.error_message
